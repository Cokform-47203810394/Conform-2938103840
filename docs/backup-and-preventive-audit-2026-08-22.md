...
| 우선순위 | 항목 | 영향 | 권고 |
|---|---|---|---|
| 해결 중 | 기본 CLI dump에서 제외되는 Auth 사용자 데이터 | 새 Supabase 프로젝트 이전 시 기존 사용자 연결이 끊길 수 있음 | 2026-08-22부터 Auth data를 암호화 백업에 명시적으로 추가. 다음 예약 백업과 실제 복구 리허설로 확인 필요 |
| P0 | 개인키·복구 번들 자동 외부 보관 없음 | 사용자가 기기와 키 백업을 같이 잃으면 E2EE 응답 복호화 불가 | 운영자에게 키 백업과 폼별 복구 번들을 서로 다른 두 장소에 보관하도록 강제 안내 |
| P1 | GitHub artifact 보관이 90일이며 한 제공자에 집중 | 장기 보관·계정 장애에 취약 | R2 이중 복제는 Cloudflare 로그인 문제로 보류. 현재는 운영자 오프라인 이중 보관과 복구 리허설을 우선 |
| P0 | 폼 즉시 삭제가 응답·암호화 버전·공개본을 FK cascade로 함께 삭제 | 다음 일일 DB 백업 전에는 운영 데이터 복구가 어려움 | 30일 휴지통(soft delete)과 작성자 복원 UI를 추가하고, 영구 삭제는 별도 확인으로 분리 |
