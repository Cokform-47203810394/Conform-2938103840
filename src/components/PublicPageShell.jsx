    ...
    <main className="min-h-screen bg-[#F5F3EC] px-4 py-7 text-[#17251F] sm:px-6 sm:py-10">
      <div className="mx-auto max-w-6xl">
        ...
        <header className="border-b border-[#DDE1D9] pb-7 sm:pb-9">
          ...
          {eyebrow && <p className="text-xs font-bold tracking-[0.08em] text-[#17866D]">{eyebrow}</p>}
...
export function DocumentCard({ children, tone = "default", className = "" }) {
  ...
  return <section className={`rounded-xl border p-5 sm:p-6 ${toneClass} ${className}`}>{children}</section>;
}

export function AnchorNav({ items }) {
  return <nav aria-label="이 페이지 목차" className="rounded-xl border border-[#DDE1D9] bg-[#FFFDF8] p-3"><p className="px-2 pb-2 pt-1 text-xs font-bold tracking-[0.08em] text-[#17866D]">이 페이지에서 보기</p><div className="flex gap-1 overflow-x-auto [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden lg:flex-col lg:overflow-visible">{items.map((item) => <a key={item.href} href={item.href} className="shrink-0 rounded-lg px-3 py-2 text-sm font-medium text-[#59645E] transition hover:bg-[#EAF6EF] hover:text-[#0B4D3D]">{item.label}</a>)}</div></nav>;
}
