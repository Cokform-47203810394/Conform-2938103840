...
export default function ServiceRestrictionsPage({ onBack }) {
  ...
  return <PublicPageShell eyebrow="COKFORM / TRUST & SAFETY" title="서비스 이용제한 정책" description="Cokform은 자유로운 폼 운영을 지지합니다. 동시에 응답자·작성자의 권리, 개인정보, 서비스 안정성을 침해하는 행위에는 필요한 범위의 조치를 적용합니다." icon={Scale} onBack={onBack} aside={<AnchorNav items={navItems} />}>
    <DocumentCard tone="notice"><div className="flex gap-3"><AlertTriangle size={20} className="mt-0.5 shrink-0" /><div><h2 className="font-semibold">파일럿 운영 정책</h2><p className="mt-1 text-sm leading-6">이 문서는 법률 검토 전 공개 초안입니다. 이용약관, 개인정보처리방침 및 관계 법령과 함께 적용되며, 충돌하는 경우 관계 법령과 개별적으로 고지된 조건을 우선합니다.</p></div></div></DocumentCard>
