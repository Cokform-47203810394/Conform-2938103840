...
import { AlertTriangle, ArrowUpRight, Globe2, LockKeyhole, MapPin, ShieldCheck } from "lucide-react";
...
export default function InternationalTransferPage({ onBack }) {
  ...
  return <PublicPageShell eyebrow="COKFORM / PRIVACY & LEGAL" title="개인정보 국외이전·국외 처리 안내" description="Cokform의 실제 국내 저장 구조와, 웹 전송·선택 연동에서 발생할 수 있는 해외 처리를 구분해 공개합니다. 확정되지 않은 처리자·계약 조건은 사실처럼 단정하지 않고 변경 시 함께 갱신합니다." icon={Globe2} onBack={onBack} aside={<AnchorNav items={navItems} />}>
    <DocumentCard tone="notice"><div className="flex gap-3"><AlertTriangle size={20} className="mt-0.5 shrink-0" /><div><h2 className="font-semibold">법률 검토 전 안내</h2><p className="mt-1 text-sm leading-6">이 페이지는 2026-08-18 기준 Cokform의 코드·배포 구성을 토대로 한 파일럿 고지입니다. 실제 사업자 정보, 외부 처리자의 계약·리전·보유기간이 확정 또는 변경되면 시행 전 갱신합니다.</p></div></div></DocumentCard>
    ...
    <section id="external" className="scroll-mt-8"><div className="mb-3 flex items-center gap-2"><ShieldCheck size={18} className="text-[#17866D]" /><h2 className="text-xl font-bold tracking-[-0.04em]">국외 처리 가능 기능</h2></div><div className="overflow-x-auto rounded-2xl border border-[#DDE1D9] bg-[#FFFDF8] shadow-[0_6px_20px_rgba(23,37,31,0.05)]"><table className="min-w-[900px] w-full text-left text-sm"><thead className="bg-[#EAF6EF] text-xs text-[#0B4D3D]"><tr>{["구분", "처리자", "처리 항목", "국가·지역", "시기·방법", "목적", "거부·효과"].map((title) => <th key={title} className="px-4 py-3 font-bold">{title}</th>)}</tr></thead><tbody className="divide-y divide-[#E7E9E3]">{rows.map((row) => <tr key={row[0]} className="align-top">{row.map((cell, index) => <td key={index} className="px-4 py-4 leading-6 text-[#59645E] first:font-semibold first:text-[#17251F]">{cell}</td>)}</tr>)}</tbody></table></div><p className="mt-3 text-xs leading-5 text-[#78837C]">Cloudflare는 전 세계 엣지 네트워크에서 트래픽을 처리할 수 있으며, 별도 데이터 현지화 기능으로 암호 해제·로그·메타데이터의 처리 위치를 제어하는 기능을 제공합니다. Cokform은 현재 무료 파일럿 구성에서 해당 기능의 적용 여부와 계약 조건을 주기적으로 점검합니다.</p></section>

    <section id="rights" className="scroll-mt-8"><div className="mb-3 flex items-center gap-2"><LockKeyhole size={18} className="text-[#17866D]" /><h2 className="text-xl font-bold tracking-[-0.04em]">거부권·보호조치·문의</h2></div><div className="grid gap-4 md:grid-cols-2"><DocumentCard><h3 className="font-semibold">선택 기능은 사용자가 결정합니다</h3><p className="mt-2 text-sm leading-6 text-[#59645E]">Google Drive·Sheets 내보내기는 사용자가 직접 실행하고 Google 권한을 허용할 때만 동작합니다. 권한을 허용하지 않으면 해당 직접 내보내기 기능만 사용할 수 없습니다.</p></DocumentCard><DocumentCard><h3 className="font-semibold">변경과 권리 행사</h3><p className="mt-2 text-sm leading-6 text-[#59645E]">처리자, 국가, 항목, 보유기간 또는 근거가 바뀌면 이 페이지와 개인정보처리방침을 갱신합니다. 문의는 <a className="font-semibold underline underline-offset-2" href="mailto:seoharo0111@gmail.com">seoharo0111@gmail.com</a>으로 보내주세요.</p></DocumentCard></div><DocumentCard className="mt-4"><h3 className="font-semibold">공식 참고자료</h3><div className="mt-3 flex flex-wrap gap-3 text-sm"><a className="inline-flex items-center gap-1 text-[#0B4D3D] underline underline-offset-2" target="_blank" rel="noreferrer" href="https://www.privacy.go.kr/front/contents/cntntsView.do?contsNo=367">개인정보 포털 국외이전 제도 <ArrowUpRight size={14} /></a><a className="inline-flex items-center gap-1 text-[#0B4D3D] underline underline-offset-2" target="_blank" rel="noreferrer" href="https://www.pipc.go.kr/np/cop/bbs/selectBoardArticle.do?bbsId=BS292&mCode=I040010000&nttId=10972">개인정보보호위원회 안내 <ArrowUpRight size={14} /></a><a className="inline-flex items-center gap-1 text-[#0B4D3D] underline underline-offset-2" target="_blank" rel="noreferrer" href="https://developers.cloudflare.com/data-localization/">Cloudflare 데이터 현지화 <ArrowUpRight size={14} /></a></div></DocumentCard></section>
  </PublicPageShell>;
}
