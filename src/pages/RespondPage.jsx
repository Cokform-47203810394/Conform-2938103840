...
export default function RespondPage({ formId }) {
  ...
  if (doc === undefined) {
    return <div className="flex min-h-screen items-center justify-center text-sm text-[#79747E]">불러오는 중…</div>;
  }
  if (doc === null) {
    return (
      <div className="flex min-h-screen items-center justify-center px-4 text-center text-sm text-[#79747E]">
        존재하지 않거나 삭제된 설문지예요.
      </div>
    );
  }
  ...
  const handleSubmit = async (answers) => {
    const fresh = await getFormDoc(formId);
    const responses = [...(fresh?.responses || []), { id: `${Date.now()}`, submittedAt: new Date().toISOString(), answers }];
    await saveFormDoc(formId, { form: fresh?.form || form, responses });
  ...
  if (closed || blocked) {
    return (
      <div className="min-h-screen px-4 py-10" style={{ backgroundColor: form.backgroundColor || "#F3EDF7" }}>
        <div className={`mx-auto max-w-xl rounded-xl border-t-8 bg-white p-8 text-center ${ELEV1}`} style={{ borderTopColor: accent }}>
          <h1 className="text-xl font-normal text-[#1C1B1F]">{form.title}</h1>
          <p className="mt-3 text-sm text-[#49454F]">
  ...
  return (
    <div className="min-h-screen px-3 py-6 sm:px-4" style={{ backgroundColor: form.backgroundColor || "#F3EDF7" }}>
